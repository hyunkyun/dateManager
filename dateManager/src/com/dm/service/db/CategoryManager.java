...
public class CategoryManager{
		...
		private static final String CLASSNAME = CategoryManager.class.getSimpleName();
		private DBHelper db;
		 ...
		 * @param to	: CategoryTO 
		 */
		public long insert(final Category to){
		 ...
		 * @param to	: CategoryTO 
		 */
		public void update(final Category to){
			ContentValues values = new ContentValues();
			
			values.put(CategoryTable.ID, to.getId());
			values.put(CategoryTable.NAME, to.getName());
			
			Log.v(TAG, CategoryManager.CLASSNAME + " update id:" + to.getId() + ", name:" +  to.getName());
		...
		public void delete(final int id){
			Log.v(TAG, CategoryManager.CLASSNAME + " delete id:" + id );
		...
		public List<Category> get(int id){
			...
			String sql = "SELECT * FROM " + CategoryTable.TABLE_NAME
					...
					+ " ORDER BY 1";
			try{
				Log.v(TAG, CategoryManager.CLASSNAME + " getList ");
				c = db.get(sql);
				ret = setBindCursorCategory(c);
				
			}catch(SQLException e){
				Log.e(TAG, CategoryManager.CLASSNAME + " getList sqlerror");
			}finally{
